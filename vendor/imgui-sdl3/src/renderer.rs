...
use imgui::{internal::RawWrapper, DrawCmdParams, DrawIdx, DrawVert, TextureId};
...
impl Renderer {
            ...
            for draw_cmd in draw_list.commands() {
                match draw_cmd {
                    imgui::DrawCmd::Elements {
                        ...
                        let sampler_binding = self.binding_for_texture(texture_id);
                        render_pass
                            .bind_fragment_samplers(0, std::slice::from_ref(&sampler_binding));
    ...
    fn binding_for_texture(&self, texture_id: TextureId) -> TextureSamplerBinding {
        if texture_id.id() == 0 {
            TextureSamplerBinding::new()
                .with_texture(&self.font_texture)
                .with_sampler(&self.font_sampler)
        } else {
            unsafe { (texture_id.id() as *const TextureSamplerBinding).read() }
        }
    }
}
