...
impl Renderer {
    ...
    pub fn render(
        ...
        render_pass.bind_fragment_samplers(0, &[font_sampler_binding]);
        ...
        let mut current_texture_id: usize = usize::MAX;
            ...
            for draw_cmd in draw_list.commands() {
                match draw_cmd {
                    imgui::DrawCmd::Elements {
                        ...
                        let requested_texture = texture_id.id() as usize;
                        if requested_texture != current_texture_id {
                            if requested_texture == 0 {
                                render_pass.bind_fragment_samplers(0, &[font_sampler_binding]);
                            } else {
                                let sampler_binding = requested_texture as *const TextureSamplerBinding;
                                if sampler_binding.is_null() {
                                    render_pass.bind_fragment_samplers(0, &[font_sampler_binding]);
                                } else {
                                    unsafe {
                                        render_pass.bind_fragment_samplers(
                                            0,
                                            std::slice::from_raw_parts(sampler_binding, 1),
                                        );
                                    }
                                }
                            }

                            current_texture_id = requested_texture;
                        }
