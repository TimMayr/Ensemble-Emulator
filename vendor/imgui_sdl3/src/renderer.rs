...
impl Renderer {
    ...
    pub fn new(device: &Device, window: &Window, imgui_context: &mut imgui::Context) -> Result<Self, Box<dyn Error>> {
        ...
        // Create a sampler for the font texture once and reuse it across frames
    ...
    pub fn render(
        ...
        let font_sampler_binding = TextureSamplerBinding::new()
            .with_texture(&self.font_texture)
            .with_sampler(&self.font_sampler);

        render_pass.bind_fragment_samplers(0, slice::from_ref(&font_sampler_binding));
        ...
        let mut last_texture_id = usize::MAX;
            ...
            for draw_cmd in draw_list.commands() {
                match draw_cmd {
                    imgui::DrawCmd::Elements {
                        ...
                        let requested_texture = texture_id.id();
                        if last_texture_id != requested_texture {
                            if requested_texture == 0 {
                                render_pass.bind_fragment_samplers(
                                    0,
                                    slice::from_ref(&font_sampler_binding),
                                );
                            } else {
                                let sampler_binding = unsafe {
                                    &*(requested_texture as *const TextureSamplerBinding)
                                };
                                render_pass.bind_fragment_samplers(
                                    0,
                                    slice::from_ref(sampler_binding),
                                );
                            }
                            last_texture_id = requested_texture;
                        }
