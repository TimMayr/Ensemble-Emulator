...
impl TraceLog {
    ...
    pub fn trace(&mut self, nes: &Nes) {
        ...
        let relevant_mem_start = cpu.program_counter - 1;
        let relevant_mem_end =
            relevant_mem_start + opcode::get_bytes_for_opcode(current_opcode) as u16;
...
pub fn get_opcode_descriptor(opcode: OpCode, cpu: &mut Cpu) -> String {
    match opcode.op_type {
        ...
        OpType::AbsoluteRead(..) | OpType::AbsoluteRMW(..) | OpType::AbsoluteWrite(..) => {
            let address = ((cpu.memory.mem_read_debug(cpu.program_counter + 1) as u16) << 8)
        ...
        | OpType::IndexedIndirectWrite(..) => {
            ...
            let lookup_addr = ((cpu.memory.mem_read_debug((effective_address + 1) as u16) as u16)
                << 8)
        ...
        | OpType::IndirectIndexedWrite(..) => {
            ...
            let effective_addr = ((cpu.memory.mem_read_debug((address + 1) as u16) as u16) << 8)
