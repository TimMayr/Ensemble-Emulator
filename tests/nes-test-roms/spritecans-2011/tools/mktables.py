...
def main(argv):
    if len(argv) >= 2 and argv[1] in ('/?', '-?', '-h', '--help'):
        print
        "usage: %s TABLENAME FILENAME" % argv[0]
        print
        "known tables:", ' '.join(sorted(tableNames))
    elif len(argv) < 3:
        print
        "mktables: too few arguments; try %s --help" % argv[0]
    elif argv[1] in tableNames:
        tableNames[argv[1]](argv[2])
    else:
        print
        "mktables: no such table %s; try %s --help" % (argv[1], argv[0])
