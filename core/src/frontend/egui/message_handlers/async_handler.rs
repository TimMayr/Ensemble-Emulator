...
use crate::emulation::messages::{EmulatorFetchable, FrontendMessage, SaveType};
...
impl EguiApp {
    ...
    fn handle_palette_loaded(
        &mut self,
        ctx: &Context,
        palette: crate::emulation::messages::RgbPalette,
    ...
    fn handle_rom_verification_failed(
        ...
        let message = match error {
            SavestateLoadError::FailedToLoadSavestate => "Unexpected error during ROM verification.",
            SavestateLoadError::FailedToComputeChecksum => {
        ...
        self.config.pending_dialogs.rom_selection_dialog = Some(RomSelectionDialogState { context });
    }
    ...
    fn handle_quickload(&mut self) {
        if let Some(path) = self.get_current_quicksave_path() {
            let savestate = match savestate::try_load_state(&path) {
                Some(s) => s,
                None => {
                    let _ = self.async_sender.send(AsyncFrontendMessage::SavestateLoadFailed(
                        SavestateLoadError::FailedToLoadSavestate,
                    ));
                    return;
                }
